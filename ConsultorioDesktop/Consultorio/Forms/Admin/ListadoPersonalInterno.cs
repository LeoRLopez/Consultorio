...
namespace Consultorio.Forms
{
    public partial class ListadoPersonalInterno : Form
    {
        ...
        private void btnEliminar_Click_1(object sender, EventArgs e)
        {
            if (dgvPersonalInterno.CurrentRow != null)
            {
                var personalInternoSeleccionado = ((PersonalInternoVM)dgvPersonalInterno.CurrentRow.DataBoundItem);
                using (var entidades = new ClinicaEntities())
                {
                    var personalInternoDB = entidades.PersonalInterno.First(x => x.IdPersonal == personalInternoSeleccionado.PersonalInternoId);
                    personalInternoDB.Bajalogica = true;
                    entidades.SaveChanges();
                    MessageBox.Show("Se ha dado de baja el personal", "Aviso", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    RefrescarGridView();
                }
            }
            else
